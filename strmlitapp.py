...
query_map = {
    "Top 10 vehicle numbers involved in drug-related stops":"SELECT vehicle_number, COUNT(*) AS drug_stop_count FROM police_stops WHERE drugs_related_stop = 1 GROUP BY vehicle_number ORDER BY drug_stop_count DESC LIMIT 10;",
    "Vehicles most frequently searched":"SELECT vehicle_number, COUNT(*) AS search_count FROM police_stops WHERE search_conducted = 1 GROUP BY vehicle_number ORDER BY search_count DESC LIMIT 10;",
    "The driver age group which had the highest arrest rate":"SELECT CASE WHEN driver_age < 18 THEN 'Under 18' WHEN driver_age BETWEEN 18 AND 24 THEN '18-24' WHEN driver_age BETWEEN 25 AND 34 THEN '25-34' WHEN driver_age BETWEEN 35 AND 44 THEN '35-44' WHEN driver_age BETWEEN 45 AND 54 THEN '45-54' ELSE '55+' END AS age_group, COUNT(*) AS total_stops, SUM(CASE WHEN is_arrested = TRUE THEN 1 ELSE 0 END) AS arrest_count, ROUND(SUM(CASE WHEN is_arrested = TRUE THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) AS arrest_rate FROM police_stops GROUP BY age_group ORDER BY arrest_rate DESC LIMIT 1;",
    "Gender distribution of drivers that are stopped in each country":"SELECT country_name, driver_gender, COUNT(*) AS stop_count FROM police_stops GROUP BY country_name, driver_gender ORDER BY country_name, driver_gender;",
    "The race and gender combination that has the highest search rate":"SELECT driver_race, driver_gender, COUNT(*) AS total_stops, SUM(search_conducted) AS searches, ROUND(SUM(search_conducted) * 100.0 / COUNT(*), 2) AS search_rate FROM police_stops GROUP BY driver_race, driver_gender ORDER BY search_rate DESC LIMIT 1;",
    "The time of day that sees the most traffic stops":"SELECT EXTRACT(HOUR FROM stop_time) AS hour_of_day, COUNT(*) AS stop_count FROM police_stops GROUP BY hour_of_day ORDER BY stop_count DESC LIMIT 1;",
    "The average stop duration for different violations":"SELECT violation, ROUND(AVG(stop_duration), 2) AS avg_duration FROM police_stops GROUP BY violation ORDER BY avg_duration DESC;",
    "Are stops during the night more likely to lead to arrests?":"SELECT CASE WHEN (SELECT SUM(is_arrested) FROM police_stops WHERE EXTRACT(HOUR FROM stop_time) BETWEEN 19 AND 23 OR EXTRACT(HOUR FROM stop_time) BETWEEN 0 AND 5) > (SELECT SUM(is_arrested) FROM police_stops WHERE EXTRACT(HOUR FROM stop_time) BETWEEN 6 AND 18) THEN 'Yes' ELSE 'No' END AS night_more_arrests;",
    "Violations that are most associated with searches or arrests":"SELECT violation, COUNT(*) AS total_stops, SUM(search_conducted) AS searches, SUM(is_arrested) AS arrests FROM police_stops GROUP BY violation ORDER BY searches + arrests DESC;",
    "Violations that are most common among younger drivers (<25)":"SELECT violation, COUNT(*) AS stop_count FROM police_stops WHERE driver_age < 25 GROUP BY violation ORDER BY stop_count DESC;",
    "Is there a violation that rarely results in search or arrest?":"SELECT CASE WHEN EXISTS (SELECT 1 FROM police_stops GROUP BY violation HAVING SUM(search_conducted) + SUM(is_arrested) < 0.05 * COUNT(*)) THEN 'Yes' ELSE 'No' END AS rarely_results_in_search_or_arrest;",
    "Countries that report the highest rate of drug-related stops":"SELECT country_name, COUNT(*) AS total_stops, SUM(drugs_related_stop) AS drug_stops, ROUND(SUM(drugs_related_stop) * 100.0 / COUNT(*), 2) AS drug_stop_rate FROM police_stops GROUP BY country_name ORDER BY drug_stop_rate DESC;",
    "Arrest rate by country and violation":"SELECT country_name, violation, COUNT(*) AS total_stops, SUM(is_arrested) AS arrests, ROUND(SUM(is_arrested) * 100.0 / COUNT(*), 2) AS arrest_rate FROM police_stops GROUP BY country_name, violation ORDER BY arrest_rate DESC;",
    "Country that has the most stops with search conducted":"SELECT country_name, COUNT(*) AS total_stops, SUM(search_conducted) AS search_stops FROM police_stops GROUP BY country_name ORDER BY search_stops DESC LIMIT 1;",
    "Yearly Breakdown of Stops and Arrests by Country":"SELECT DISTINCT country_name, year, COUNT(*) OVER (PARTITION BY country_name, year) AS total_stops, SUM(is_arrested) OVER (PARTITION BY country_name, year) AS arrests FROM (SELECT country_name, EXTRACT(YEAR FROM stop_date) AS year, is_arrested FROM police_stops) AS sub ORDER BY country_name, year;",
    "Driver Violation Trends Based on Age and Race":"SELECT a.driver_race, b.age_group, a.violation, COUNT(*) AS stop_count FROM police_stops a JOIN (SELECT id, CASE WHEN driver_age < 18 THEN 'Under 18' WHEN driver_age BETWEEN 18 AND 24 THEN '18-24' WHEN driver_age BETWEEN 25 AND 34 THEN '25-34' WHEN driver_age BETWEEN 35 AND 44 THEN '35-44' WHEN driver_age BETWEEN 45 AND 54 THEN '45-54' ELSE '55+' END AS age_group FROM police_stops) b ON a.id = b.id GROUP BY a.driver_race, b.age_group, a.violation ORDER BY a.driver_race, b.age_group, stop_count DESC;",
    "Time Period Analysis of Stops (Year, Month, Hour)":"SELECT a.year, a.month, a.hour, COUNT(*) AS stop_count FROM police_stops b JOIN (SELECT id, EXTRACT(YEAR FROM stop_date) AS year, EXTRACT(MONTH FROM stop_date) AS month, EXTRACT(HOUR FROM stop_time) AS hour FROM police_stops) a ON a.id = b.id GROUP BY a.year, a.month, a.hour ORDER BY a.year, a.month, a.hour;",
    "Violations with High Search and Arrest Rates":"SELECT DISTINCT violation, COUNT(*) OVER (PARTITION BY violation) AS total_stops, SUM(search_conducted) OVER (PARTITION BY violation) AS searches, SUM(is_arrested) OVER (PARTITION BY violation) AS arrests, ROUND(SUM(search_conducted) OVER (PARTITION BY violation) * 100.0 / COUNT(*) OVER (PARTITION BY violation), 2) AS search_rate, ROUND(SUM(is_arrested) OVER (PARTITION BY violation) * 100.0 / COUNT(*) OVER (PARTITION BY violation), 2) AS arrest_rate FROM police_stops ORDER BY search_rate DESC, arrest_rate DESC;",
    "Driver Demographics by Country (Age, Gender, and Race)":"SELECT country_name, driver_gender, driver_race, ROUND(AVG(driver_age), 1) AS avg_age, COUNT(*) AS stop_count FROM police_stops GROUP BY country_name, driver_gender, driver_race ORDER BY country_name, driver_gender, driver_race;",
    "Top 5 Violations with Highest Arrest Rates":"SELECT violation, COUNT(*) AS total_stops, SUM(is_arrested) AS arrests, ROUND(SUM(is_arrested) * 100.0 / COUNT(*), 2) AS arrest_rate FROM police_stops GROUP BY violation ORDER BY arrest_rate DESC LIMIT 5;"
}
